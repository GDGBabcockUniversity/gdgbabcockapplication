> **Repo focus — gdgbabcockapplication (static v0 recruitment microsite):** mostly **reference** for this effort — it holds no event system. Useful only as brand/IA reference (the `PRODUCTS` list, org-tree pattern). No core workstream lives here; included so the full platform context is available if this site later consumes shared identity/events.
>
